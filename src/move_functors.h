...
struct move_insert
{
	...
	double attempt()
	{
		...
		for (int i = 0; i < N; ++i)
		{
			double tau = config.param.beta * rng();
...
struct move_ZtoW2
{
	...
	double attempt()
	{
		...
		return config.param.W2toZ / config.param.ZtoW2
			* config.l.parity(s1) * config.l.parity(s2) * config.param.zeta2
			* config.param.ratio_w2 * det_ratio;
	}
	...
	double attempt()
	{
		...
		return config.param.ZtoW2 / config.param.W2toZ
			* config.l.parity(sites[0]) * config.l.parity(sites[1])
			/ config.param.zeta2 / config.param.ratio_w2 * det_ratio;
	}
...
struct move_W2toW4
{
	...
	double attempt()
	{
		...
		return config.param.W4toW2 / config.param.W2toW4
			* config.l.parity(s1) * config.l.parity(s2) * config.param.zeta4
			/ config.param.zeta2 * config.param.ratio_w2 * det_ratio;
	}
...
struct move_W4toW2
{
	...
	double attempt()
	{
		...
		return config.param.W2toW4 / config.param.W4toW2
			* config.l.parity(sites[0]) * config.l.parity(sites[1])
			* config.param.zeta2 / config.param.zeta4
			/ config.param.ratio_w2 * det_ratio;
	}
...
struct move_ZtoW4
{
	...
	double attempt()
	{
		...
		return config.param.W4toZ / config.param.ZtoW4 * config.l.parity(s1)
			...
			* config.param.zeta4 * config.param.ratio_w4 * det_ratio;
	}
...
struct move_shift
{
	...
	double attempt()
	{
		...
		double tau_shift = -0.05*config.param.beta + 0.1*config.param.beta
			* rng();
		if (worm_vert[0].tau + tau_shift > config.param.beta)
			tau_shift -= config.param.beta;
